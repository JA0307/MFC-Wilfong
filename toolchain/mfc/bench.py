...
def bench(targets=None):
    ...
    cons.print()
    cons.print(
        f"[bold]Benchmarking {format_list_to_string(ARG('targets'), 'magenta')} ([magenta]{os.path.relpath(bench_dirpath)}[/magenta]):[/bold]"
    )
    cons.indent()
    ...
    try:
        ...
        for i, case in enumerate(CASES):
            ...
            cons.print(
                f"{str(i + 1).zfill(len(CASES) // 10 + 1)}/{len(CASES)}: {case.slug} @ [bold]{os.path.relpath(case.path)}[/bold]"
            )
            cons.indent()
            cons.print()
                    ...
                    try:
                        ...
                        if rc != 0:
                            if attempt < max_attempts:
                                cons.print(
                                    f"[bold yellow]WARNING[/bold yellow]: Case {case.slug} failed with exit code {rc} (attempt {attempt}/{max_attempts})"
                                )
                                cons.print("Retrying in 5s...")
                                time.sleep(5)
                                continue
                            cons.print(
                                f"[bold red]ERROR[/bold red]: Case {case.slug} failed with exit code {rc}"
                            )
                            cons.print(
                                f"[bold red]      Check log at: {log_filepath}[/bold red]"
                            )
                        ...
                        if not os.path.exists(summary_filepath):
                            if attempt < max_attempts:
                                cons.print(
                                    f"[bold yellow]WARNING[/bold yellow]: Summary file not created for {case.slug} (attempt {attempt}/{max_attempts})"
                                )
                                cons.print("Retrying in 5s...")
                                time.sleep(5)
                                continue
                            cons.print(
                                f"[bold red]ERROR[/bold red]: Summary file not created for {case.slug}"
                            )
                            cons.print(
                                f"[bold red]      Expected: {summary_filepath}[/bold red]"
                            )
                            failed_cases.append(case.slug)
                            break
                        ...
                        for target in targets:
                            if target.name not in summary:
                                cons.print(
                                    f"[bold red]ERROR[/bold red]: Target {target.name} missing from summary for {case.slug}"
                                )
                            ...
                            if "exec" not in summary[target.name]:
                                cons.print(
                                    f"[bold red]ERROR[/bold red]: 'exec' time missing for {target.name} in {case.slug}"
                                )
                                validation_failed = True
                                break
                            ...
                            if (
                                ...
                                and "grind" not in summary[target.name]
                            ):
                                cons.print(
                                    f"[bold red]ERROR[/bold red]: 'grind' time missing for simulation in {case.slug}"
                                )
                                validation_failed = True
                                break
                        ...
                        cons.print(
                            f"[bold green]✓[/bold green] Case {case.slug} completed successfully"
                        )
                        break
                        ...
                        if attempt < max_attempts:
                            cons.print(
                                f"[bold yellow]WARNING[/bold yellow]: Unexpected error running {case.slug} (attempt {attempt}/{max_attempts}): {e}"
                            )
                            cons.print("Retrying in 5s...")
                            time.sleep(5)
                            continue
                        cons.print(
                            f"[bold red]ERROR[/bold red]: Unexpected error running {case.slug}: {e}"
                        )
        ...
        cons.print(
            f"Wrote results to [bold magenta]{os.path.relpath(ARG('output'))}[/bold magenta]."
        )
        ...
        if failed_cases:
            ...
            cons.print()
            raise MFCException(
                f"Benchmarking failed: {len(failed_cases)}/{len(CASES)} cases failed"
            )
...
[bold yellow]Warning[/bold yellow]: Cases in lhs and rhs are not equal.
    ...
    * lhs cases: {", ".join(set(lhs["cases"].keys()) - slugs)}.
    ...
    err = 0
    for slug in slugs:
        lhs_summary, rhs_summary = (
            lhs["cases"][slug]["output_summary"],
            rhs["cases"][slug]["output_summary"],
        )
        ...
        for i, target in enumerate(sorted(DEFAULT_TARGETS, key=lambda t: t.runOrder)):
            if (target.name not in lhs_summary) or (target.name not in rhs_summary):
                cons.print(
                    f"{target.name} not present in lhs_summary or rhs_summary - Case: {slug}"
                )
                err = 1
                continue

            if not math.isfinite(lhs_summary[target.name]["exec"]) or not math.isfinite(
                rhs_summary[target.name]["exec"]
            ):
                err = 1
                cons.print(
                    f"lhs_summary or rhs_summary reports non-real exec time for {target.name} - Case: {slug}"
                )
            try:
                exec_time_value = (
                    lhs_summary[target.name]["exec"] / rhs_summary[target.name]["exec"]
                )
                if exec_time_value < 0.9:
                    cons.print(
                        f"[bold yellow]Warning[/bold yellow]: Exec time speedup for {target.name} is less than 0.9 - Case: {slug}"
                    )
                speedups[i] = f"Exec: {exec_time_value:.2f}"
                if target == SIMULATION:
                    if not math.isfinite(
                        lhs_summary[target.name]["grind"]
                    ) or not math.isfinite(rhs_summary[target.name]["grind"]):
                        err = 1
                        cons.print(
                            f"lhs_summary or rhs_summary reports non-real grind time for {target.name} - Case: {slug}"
                        )

                    grind_time_value = (
                        lhs_summary[target.name]["grind"]
                        / rhs_summary[target.name]["grind"]
                    )
                    speedups[i] += f" & Grind: {grind_time_value:.2f}"
                    if grind_time_value < 0.95:
                        cons.print(
                            f"[bold yellow]Warning[/bold yellow]: Grind time speedup for {target.name} below threshold (<0.95) - Case: {slug}"
                        )
            except Exception as e:
                cons.print(
