...
class CaseValidator:
    ...
    def check_parameter_types(self):
        ...
        if m is not None and m > 0:
            self.prohibit(
                not self.is_set("x_domain%beg"), "x_domain%beg must be set when m > 0"
            )
            self.prohibit(
                not self.is_set("x_domain%end"), "x_domain%end must be set when m > 0"
            )
    ...
    def check_simulation_domain(self):
        ...
        self.prohibit(
            cyl_coord and p is not None and p > 0 and p % 2 == 0,
            "p must be odd for cylindrical coordinates",
        )
        self.prohibit(
            n is not None and p is not None and n == 0 and p > 0, "p must be 0 if n = 0"
        )
    ...
    def check_model_eqns_and_num_fluids(self):
        ...
        self.prohibit(
            model_eqns is not None and model_eqns not in [1, 2, 3, 4],
            "model_eqns must be 1, 2, 3, or 4",
        )
        self.prohibit(
            num_fluids is not None and num_fluids < 1, "num_fluids must be positive"
        )
        self.prohibit(
            model_eqns == 1 and num_fluids is not None,
            "num_fluids is not supported for model_eqns = 1",
        )
        self.prohibit(
            model_eqns == 2 and num_fluids is None,
            "5-equation model (model_eqns = 2) requires num_fluids to be set",
        )
        self.prohibit(
            model_eqns == 3 and num_fluids is None,
            "6-equation model (model_eqns = 3) requires num_fluids to be set",
        )
        self.prohibit(
            model_eqns == 4 and num_fluids is None,
            "4-equation model (model_eqns = 4) requires num_fluids to be set",
        )
        self.prohibit(
            model_eqns == 1 and mpp_lim, "model_eqns = 1 does not support mpp_lim"
        )
        self.prohibit(
            num_fluids == 1 and mpp_lim, "num_fluids = 1 does not support mpp_lim"
        )
        self.prohibit(
            model_eqns == 3 and cyl_coord and p != 0,
    ...
    def check_igr(self):
        ...
        self.prohibit(
            igr_pres_lim and not igr, "igr_pres_lim requires igr to be enabled"
        )
        ...
        if igr_order:
            ...
            self.prohibit(
                n is not None and n > 0 and n + 1 < igr_order,
                f"n must be at least igr_order - 1 (= {igr_order - 1})",
            )
            self.prohibit(
                p is not None and p > 0 and p + 1 < igr_order,
    ...
    def check_weno(self):
        ...
        self.prohibit(
            weno_order not in [1, 3, 5, 7], "weno_order must be 1, 3, 5, or 7"
        )
        self.prohibit(
            m + 1 < weno_order,
        ...
        self.prohibit(
            n is not None and n > 0 and n + 1 < weno_order,
            f"For 2D simulation, n must be at least weno_order - 1 (= {weno_order - 1})",
        )
        self.prohibit(
            p is not None and p > 0 and p + 1 < weno_order,
    ...
    def check_muscl(self):
        ...
        self.prohibit(
            n is not None and n > 0 and n + 1 < muscl_order,
            f"For 2D simulation, n must be at least muscl_order - 1 (= {muscl_order - 1})",
        )
        self.prohibit(
            p is not None and p > 0 and p + 1 < muscl_order,
    ...
    def check_boundary_conditions(self):
        ...
        for dir, var in [("x", "m"), ("y", "n"), ("z", "p")]:
            ...
            for bound in ["beg", "end"]:
                ...
                self.prohibit(
                    var_val is not None and var_val == 0 and bc_val is not None,
                    f"{bc_key} is not supported for {var} = 0",
                )
                self.prohibit(
                    var_val is not None and var_val > 0 and bc_val is None,
            ...
            if beg_bc is not None and end_bc is not None:
                self.prohibit(
                    (beg_bc == -1 and end_bc != -1) or (end_bc == -1 and beg_bc != -1),
            ...
            for bound in ["beg", "end"]:
                ...
                if not skip_check and bc_val is not None:
                    self.prohibit(
                        bc_val > -1 or bc_val < -17,
                        f"{bc_key} must be between -1 and -17",
                    )
                    self.prohibit(
                        bc_val == -14 and not cyl_coord,
        ...
        for dir in ["x", "y", "z"]:
            for bound in ["beg", "end"]:
                ...
                self.prohibit(
                    bc_val == -13, "Boundary condition -13 (BC_NULL) is not supported"
                )
        ...
        if cyl_coord:
            ...
            if p is not None and p > 0:
                self.prohibit(
                    bc_z_beg is not None and bc_z_beg not in [-1, -2],
                    "bc_z%beg must be -1 (periodic) or -2 (reflective) for 3D cylindrical coordinates",
                )
                self.prohibit(
                    bc_z_end is not None and bc_z_end not in [-1, -2],
    ...
    def check_bubbles_euler(self):
        ...
        self.prohibit(
            nb is None or nb < 1, "The Ensemble-Averaged Bubble Model requires nb >= 1"
        )
        self.prohibit(
            polydisperse and nb == 1, "Polydisperse bubble dynamics requires nb > 1"
        )
        self.prohibit(
            polydisperse and nb is not None and nb % 2 == 0,
        ...
        self.prohibit(
            model_eqns == 4 and rhoref is None,
            "rhoref must be set if using bubbles_euler with model_eqns = 4",
        )
        self.prohibit(
            rhoref is not None and rhoref <= 0,
            "rhoref (reference density) must be positive",
        )
        self.prohibit(
            model_eqns == 4 and pref is None,
            "pref must be set if using bubbles_euler with model_eqns = 4",
        )
        self.prohibit(
            pref is not None and pref <= 0, "pref (reference pressure) must be positive"
        )
        self.prohibit(
            model_eqns == 4 and num_fluids != 1,
        ...
        if R0ref is not None:
            self.prohibit(
                R0ref <= 0, "bub_pp%R0ref (reference bubble radius) must be positive"
            )
        if p0ref is not None:
            self.prohibit(
                p0ref <= 0, "bub_pp%p0ref (reference pressure) must be positive"
            )
        if rho0ref is not None:
            self.prohibit(
                rho0ref <= 0, "bub_pp%rho0ref (reference density) must be positive"
            )
        if T0ref is not None:
            self.prohibit(
                T0ref <= 0, "bub_pp%T0ref (reference temperature) must be positive"
            )
        ...
        if mu_l is not None:
            self.prohibit(
                mu_l < 0, "bub_pp%mu_l (liquid viscosity) must be non-negative"
            )
        if mu_g is not None:
            self.prohibit(mu_g < 0, "bub_pp%mu_g (gas viscosity) must be non-negative")
        if mu_v is not None:
            self.prohibit(
                mu_v < 0, "bub_pp%mu_v (vapor viscosity) must be non-negative"
            )
    ...
    def check_qbmm_and_polydisperse(self):
        ...
        self.prohibit(
            polydisperse and not bubbles_euler,
            "Polydisperse bubble modeling requires the bubbles_euler flag to be set",
        )
        self.prohibit(
            polydisperse and poly_sigma is None,
            "Polydisperse bubble modeling requires poly_sigma to be set",
        )
        self.prohibit(
            polydisperse and poly_sigma is not None and poly_sigma <= 0,
            "poly_sigma must be positive",
        )
        self.prohibit(
            qbmm and not bubbles_euler, "QBMM requires the bubbles_euler flag to be set"
        )
        self.prohibit(
            qbmm and nnode is not None and nnode != 4, "QBMM requires nnode = 4"
        )
    ...
    def check_hypoelasticity(self):
        ...
        self.prohibit(
            model_eqns is not None and model_eqns != 2,
            "hypoelasticity requires model_eqns = 2",
        )
        self.prohibit(
            riemann_solver is not None and riemann_solver != 1,
    ...
    def check_phase_change(self):
        ...
        self.prohibit(
            (
                ...
                or (model_eqns == 2 and relax_model not in (5, 6))
                or (model_eqns == 3 and relax_model not in (1, 4, 5, 6))
            ),
            "phase change requires model_eqns==2 with relax_model in [5,6] or model_eqns==3 with relax_model in [1,4,5,6]",
        )
        self.prohibit(
            palpha_eps is not None and palpha_eps <= 0, "palpha_eps must be positive"
        )
        self.prohibit(
            palpha_eps is not None and palpha_eps >= 1, "palpha_eps must be less than 1"
        )
        self.prohibit(
            ptgalpha_eps is not None and ptgalpha_eps <= 0,
            "ptgalpha_eps must be positive",
        )
        self.prohibit(
            ptgalpha_eps is not None and ptgalpha_eps >= 1,
    ...
    def check_ibm(self):
        ...
        self.prohibit(
            ib and n <= 0, "Immersed Boundaries do not work in 1D (requires n > 0)"
        )
    ...
    def check_mhd(self):
        ...
        self.prohibit(
            Bx0 is not None and not mhd, "Bx0 must not be set if MHD is not enabled"
        )
        self.prohibit(
            mhd and n is not None and n == 0 and Bx0 is None,
            "Bx0 must be set in 1D MHD simulations",
        )
        self.prohibit(
            mhd and n is not None and n > 0 and Bx0 is not None,
    ...
    def check_time_stepping(self):
        ...
        self.prohibit(
            time_stepper is not None and (time_stepper < 1 or time_stepper > 3),
        ...
        if variable_dt:
            ...
            self.prohibit(
                cfl_target is not None and (cfl_target <= 0 or cfl_target > 1),
            ...
            self.prohibit(
                t_save is not None and t_stop is not None and t_save > t_stop,
                "t_save must be <= t_stop",
            )
            self.prohibit(
                n_start is not None and n_start < 0, "n_start must be non-negative"
            )
        ...
        self.prohibit(
            t_step_start is not None and t_step_start < 0,
            "t_step_start must be non-negative",
        )
        self.prohibit(
            t_step_stop is not None and t_step_stop < 0,
            "t_step_stop must be non-negative",
        )
        self.prohibit(
            t_step_stop is not None
            and t_step_start is not None
            and t_step_stop <= t_step_start,
            "t_step_stop must be > t_step_start",
        )
        self.prohibit(
            t_step_save is not None and t_step_save <= 0, "t_step_save must be positive"
        )
        self.prohibit(
            t_step_save is not None
            and t_step_stop is not None
            and t_step_start is not None
            and t_step_save > t_step_stop - t_step_start,
        ...
        if not variable_dt:
            ...
            uses_fixed_stepping = self.is_set("t_step_start") or self.is_set(
                "t_step_stop"
            )
            self.prohibit(
                uses_fixed_stepping and not adap_dt and not self.is_set("dt"),
    ...
    def check_muscl_simulation(self):
        ...
        self.prohibit(
            muscl_order == 2 and muscl_lim is None,
            "muscl_lim must be defined if using muscl_order = 2",
        )
        self.prohibit(
            muscl_lim is not None and (muscl_lim < 1 or muscl_lim > 5),
    ...
    def check_model_eqns_simulation(self):
        ...
        self.prohibit(
            avg_state is not None and avg_state != 2,
    ...
    def check_bubbles_euler_simulation(self):
        ...
        self.prohibit(
            bubbles_euler and bubbles_lagrange,
        ...
        self.prohibit(
            riemann_solver is not None and riemann_solver != 2,
        ...
        self.prohibit(
            model_eqns == 2 and bubble_model == 1,
    ...
    def check_viscosity(self):
        ...
        for i in range(1, num_fluids + 1):
            ...
            for j, Re_val in [(1, Re1), (2, Re2)]:
                if Re_val is not None:
                    self.prohibit(
                        Re_val <= 0, f"fluid_pp({i})%Re({j}) must be positive"
                    )
                    self.prohibit(
                        model_eqns == 1,
        ...
        self.prohibit(
            weno_Re_flux and not viscous, "weno_Re_flux requires viscous to be enabled"
        )
    ...
    def check_mhd_simulation(self):
        ...
        self.prohibit(
            mhd and riemann_solver is not None and riemann_solver not in [1, 4],
            "MHD simulations require riemann_solver = 1 (HLL) or riemann_solver = 4 (HLLD)",
        )
        self.prohibit(
            mhd and wave_speeds is not None and wave_speeds == 2,
            "MHD requires wave_speeds = 1",
        )
        self.prohibit(
            riemann_solver == 4 and not mhd,
            "HLLD (riemann_solver = 4) is only available for MHD simulations",
        )
        self.prohibit(
            riemann_solver == 4 and relativity,
            "HLLD is not available for RMHD (relativity)",
        )
        self.prohibit(
            hyper_cleaning and not mhd, "Hyperbolic cleaning requires mhd to be enabled"
        )
        self.prohibit(
            hyper_cleaning and n is not None and n == 0,
    ...
    def check_igr_simulation(self):
        ...
        self.prohibit(
            num_igr_iters is not None and num_igr_iters < 0,
            "num_igr_iters must be greater than or equal to 0",
        )
        self.prohibit(
            num_igr_warm_start_iters is not None and num_igr_warm_start_iters < 0,
            "num_igr_warm_start_iters must be greater than or equal to 0",
        )
        self.prohibit(
            igr_iter_solver is not None and igr_iter_solver not in [1, 2],
            "igr_iter_solver must be 1 or 2",
        )
        self.prohibit(
            alf_factor is not None and alf_factor < 0, "alf_factor must be non-negative"
        )
        self.prohibit(
            model_eqns is not None and model_eqns != 2,
        ...
        self.prohibit(
            bubbles_lagrange, "IGR does not support Euler-Lagrange bubble models"
        )
        ...
        self.prohibit(
            cyl_coord, "IGR does not support cylindrical or axisymmetric coordinates"
        )
    ...
    def check_acoustic_source(self):
        ...
        self.prohibit(
            num_source is None, "num_source must be specified for acoustic_source"
        )
        self.prohibit(
            num_source is not None and num_source < 0, "num_source must be non-negative"
        )
        ...
        for j in range(1, num_source + 1):
            ...
            if support is not None:
                if dim == 1:
                    ...
                    if support == 3:
                        ...
                        self.prohibit(
                            loc[0] is None or loc[1] is None or loc[2] is None,
            ...
            self.prohibit(
                pulse is not None and pulse not in [1, 2, 3, 4],
            ...
            if pulse in [1, 3]:
                ...
                self.prohibit(
                    wavelength is not None and wavelength <= 0,
            ...
            if pulse == 2:
                ...
                self.prohibit(
                    gauss_sigma_time is not None and gauss_sigma_time <= 0,
                    f"acoustic({jstr})%gauss_sigma_time must be positive",
                )
                self.prohibit(
                    gauss_sigma_dist is not None and gauss_sigma_dist <= 0,
            ...
            self.prohibit(
                support is not None
                and support >= 5
                and npulse is not None
                and not isinstance(npulse, int),
                f"acoustic({jstr})%npulse must be an integer for support >= 5 (non-planar)",
            )
            self.prohibit(
                npulse is not None and npulse >= 5 and dipole,
                f"acoustic({jstr})%dipole is not supported for npulse >= 5",
            )
            self.prohibit(
                support is not None and support < 5 and dir_val is None,
                f"acoustic({jstr})%dir must be specified for support < 5 (planar)",
            )
            self.prohibit(
                support == 1 and dir_val is not None and dir_val == 0,
                f"acoustic({jstr})%dir must be non-zero for support = 1",
            )
            self.prohibit(
                pulse == 3 and support is not None and support >= 5,
            ...
            if support is not None and support >= 5:
                ...
                self.prohibit(
                    foc_length is not None and foc_length <= 0,
                ...
                self.prohibit(
                    aperture is not None and aperture <= 0,
    ...
    def check_adaptive_time_stepping(self):
        ...
        self.prohibit(
            time_stepper is not None and time_stepper != 3,
        ...
        self.prohibit(
            not polytropic and not bubbles_lagrange,
            "adap_dt requires polytropic = T or bubbles_lagrange = T",
        )
        self.prohibit(
            not adv_n and not bubbles_lagrange,
    ...
    def check_alt_soundspeed(self):
        ...
        self.prohibit(
            bubbles_euler, "alt_soundspeed is not compatible with bubbles_euler"
        )
        self.prohibit(
            avg_state is not None and avg_state != 2,
        ...
        self.prohibit(
            num_fluids is not None and num_fluids not in [2, 3],
    ...
    def check_bubbles_lagrange(self):
        ...
        self.prohibit(
            file_per_process, "file_per_process must be false for bubbles_lagrange"
        )
        self.prohibit(
            model_eqns == 3,
        ...
        self.prohibit(
            thermal is not None and thermal != 3,
            "bubbles_lagrange requires thermal = 3",
        )
        self.prohibit(
            cluster_type is not None and cluster_type >= 2 and smooth_type != 1,
    ...
    def check_continuum_damage(self):
        ...
        self.prohibit(
            cont_damage_s is None, "cont_damage_s must be specified for cont_damage"
        )
    ...
    def check_probe_integral_output(self):
        ...
        self.prohibit(
            probe_wrt and fd_order is None, "fd_order must be specified for probe_wrt"
        )
        self.prohibit(
            integral_wrt and fd_order is None,
            "fd_order must be specified for integral_wrt",
        )
        self.prohibit(
            integral_wrt and not bubbles_euler,
    ...
    def check_hyperelasticity(self):
        ...
        self.prohibit(
            pre_stress and not hyperelasticity,
        ...
        self.prohibit(
            model_eqns == 1, "hyperelasticity is not supported for model_eqns = 1"
        )
        self.prohibit(
            model_eqns is not None and model_eqns > 3,
    ...
    def check_restart(self):
        ...
        self.prohibit(
            not old_grid and old_ic, "old_ic can only be enabled with old_grid enabled"
        )
        self.prohibit(
            old_grid and t_step_old is None, "old_grid requires t_step_old to be set"
        )
        self.prohibit(num_patches < 0, "num_patches must be non-negative")
        self.prohibit(
            num_patches == 0 and t_step_old is None,
    ...
    def check_qbmm_pre_process(self):
        ...
        self.prohibit(
            dist_type is not None
            and dist_type != 1
            and rhoRV is not None
            and rhoRV > 0,
    ...
    def check_parallel_io_pre_process(self):
        ...
        if down_sample:
            ...
            self.prohibit(
                not file_per_process, "down sample requires file_per_process = T"
            )
            if m is not None and m >= 0:
    ...
    def check_grid_stretching(self):
        ...
        for direction in ["x", "y", "z"]:
            ...
            self.prohibit(
                old_grid, f"old_grid and stretch_{direction} are incompatible"
            )
            self.prohibit(
                a is None, f"a_{direction} must be set with stretch_{direction} enabled"
            )
            self.prohibit(
                coord_a is None,
    ...
    def check_perturb_density(self):
        ...
        if perturb_flow:
            self.prohibit(
                perturb_flow_fluid is None or perturb_flow_mag is None,
                "perturb_flow_fluid and perturb_flow_mag must be set with perturb_flow = T",
            )
        else:
            self.prohibit(
                perturb_flow_fluid is not None or perturb_flow_mag is not None,
    ...
    def check_misc_pre_process(self):
        ...
        self.prohibit(
            mixlayer_vel_profile and n == 0, "mixlayer_vel_profile requires n > 0"
        )
    ...
    def check_patch_physics(self):
        ...
        for i in range(1, num_patches + 1):
            ...
            alter_patches = [
                self.get(f"patch_icpp({i})%alter_patch({j})") == "T"
            ...
            if bubbles_euler:
                ...
                if r0 is not None and self._is_numeric(r0):
                    self.prohibit(
                        r0 <= 0, f"patch_icpp({istr})%r0 must be positive (got {r0})"
                    )
    ...
    def check_bc_patches(self):
        ...
        self.prohibit(
            num_bc_patches > num_bc_patches_max,
        ...
        for i in range(1, num_bc_patches + 1):
            ...
            if geometry == 1:
                ...
                if direction in [1, 2]:
                    self.prohibit(
                        centroid[direction - 1] is not None or centroid[2] is not None,
                        f"Line Segment Patch {i} of Dir {direction} can't have centroid in Dir {direction} or 3",
                    )
                    self.prohibit(
                        length[direction - 1] is not None or length[2] is not None,
            ...
            # Circle BC (geometry = 2)
                ...
                self.prohibit(
                    radius is None, f"Circle Patch {i} must have radius defined"
                )
                self.prohibit(
                    any(length_val is not None for length_val in length),
                    f"Circle Patch {i} can't have lengths defined",
                )
                if direction in [1, 2, 3]:
                    self.prohibit(
                        centroid[direction - 1] is not None,
            ...
            # Rectangle BC (geometry = 3)
                ...
                self.prohibit(
                    radius is not None, f"Rectangle Patch {i} can't have radius defined"
                )
                if direction in [1, 2, 3]:
                    self.prohibit(
                        centroid[direction - 1] is not None,
                    ...
                    self.prohibit(
                        length[direction - 1] is not None,
            ...
            if bc_type is not None:
                ...
                self.prohibit(
                    (-14 <= bc_type <= -4) or bc_type == -1 or bc_type < -17,
    ...
    def check_vorticity(self):
        ...
        self.prohibit(
            n is not None and n == 0 and any(omega_wrt),
            "omega_wrt requires n > 0 (at least 2D)",
        )
        self.prohibit(
            p is not None and p == 0 and (omega_wrt[0] or omega_wrt[1]),
            "omega_wrt(1) and omega_wrt(2) require p > 0 (3D)",
        )
        self.prohibit(
            any(omega_wrt) and fd_order is None, "fd_order must be set for omega_wrt"
        )
    ...
    def check_schlieren(self):
        ...
        self.prohibit(
            n is not None and n == 0 and schlieren_wrt,
            "schlieren_wrt requires n > 0 (at least 2D)",
        )
        self.prohibit(
            schlieren_wrt and fd_order is None, "fd_order must be set for schlieren_wrt"
        )
    ...
    def check_partial_domain(self):
        ...
        self.prohibit(
            x_output_beg is None or x_output_end is None,
        ...
        if n is not None and n != 0:
            ...
            self.prohibit(
                y_output_beg is None or y_output_end is None,
        ...
        if p is not None and p != 0:
            ...
            self.prohibit(
                z_output_beg is None or z_output_end is None,
    ...
    def check_fft(self):
        ...
        if m_glb is not None and n_glb is not None and p_glb is not None:
            self.prohibit(
                (m_glb + 1) % 2 != 0 or (n_glb + 1) % 2 != 0 or (p_glb + 1) % 2 != 0,
    ...
    def check_surface_tension_post(self):
        ...
        self.prohibit(
            cf_wrt and not surface_tension,
    ...
    def check_no_flow_variables(self):
        ...
        if num_fluids is None:
            num_fluids = 1
        alpha_rho_wrt_any = any(
            self.get(f"alpha_rho_wrt({i})", "F") == "T"
            for i in range(1, num_fluids + 1)
        )
        ...
        alpha_wrt_any = any(
            self.get(f"alpha_wrt({i})", "F") == "T" for i in range(1, num_fluids + 1)
        )
        omega_wrt_any = any(
            self.get(f"omega_wrt({i})", "F") == "T" for i in range(1, 4)
        )
    ...
    def check_volume_fraction_sum(self):
        ...
        if (
            not self._is_numeric(num_patches)
            or num_patches <= 0
            or not self._is_numeric(num_fluids)
        ):
            return
    ...
    def check_alpha_rho_consistency(self):
        ...
        for i in range(1, num_patches + 1):
            ...
            for j in range(1, num_fluids + 1):
                ...
                self.warn(
                    alpha == 0 and alpha_rho != 0,
                    f"patch_icpp({i}): alpha({j}) = 0 but alpha_rho({j}) = {alpha_rho} (density in absent phase)",
                )
                self.warn(
                    alpha > 1e-10 and alpha_rho == 0,
                    f"patch_icpp({i}): alpha({j}) = {alpha} but alpha_rho({j}) = 0 (present phase has zero density)",
                )
    ...
    def check_patch_within_domain(self):
        ...
        if (
            self.get("stretch_x", "F") == "T"
            or self.get("stretch_y", "F") == "T"
            or self.get("stretch_z", "F") == "T"
        ):
            return
        ...
        x_bounds_ok = (
            ...
            and self._is_numeric(x_beg)
            and self._is_numeric(x_end)
        )
        y_bounds_ok = (
            y_beg is not None
            and y_end is not None
            and self._is_numeric(y_beg)
            and self._is_numeric(y_end)
        )
        z_bounds_ok = (
            z_beg is not None
            and z_end is not None
            and self._is_numeric(z_beg)
            and self._is_numeric(z_end)
        )
        ...
        for i in range(1, num_patches + 1):
            ...
            if has_x and x_bounds_ok and self._is_numeric(xc) and self._is_numeric(lx):
                ...
                self.prohibit(
                    patch_x_hi < x_beg or patch_x_lo > x_end,
                    f"patch_icpp({i}): x-extent [{patch_x_lo}, {patch_x_hi}] is entirely outside domain [{x_beg}, {x_end}]",
                )
            ...
            if geometry in [3, 9] and y_bounds_ok:
                ...
                if (
                    yc is not None
                    and ly is not None
                    and self._is_numeric(yc)
                    and self._is_numeric(ly)
                ):
                    patch_y_lo = yc - ly / 2.0
                    patch_y_hi = yc + ly / 2.0
                    self.prohibit(
                        patch_y_hi < y_beg or patch_y_lo > y_end,
                        f"patch_icpp({i}): y-extent [{patch_y_lo}, {patch_y_hi}] is entirely outside domain [{y_beg}, {y_end}]",
                    )
            ...
            if geometry == 9 and z_bounds_ok:
                ...
                if (
                    zc is not None
                    and lz is not None
                    and self._is_numeric(zc)
                    and self._is_numeric(lz)
                ):
                    patch_z_lo = zc - lz / 2.0
                    patch_z_hi = zc + lz / 2.0
                    self.prohibit(
                        patch_z_hi < z_beg or patch_z_lo > z_end,
                        f"patch_icpp({i}): z-extent [{patch_z_lo}, {patch_z_hi}] is entirely outside domain [{z_beg}, {z_end}]",
                    )
    ...
    def check_eos_parameter_sanity(self):
        ...
        for i in range(1, int(num_fluids) + 1):
            ...
            self.warn(
                gamma < 0.1,
                f"fluid_pp({i})%gamma = {gamma} implies physical gamma = {physical_gamma:.2f} (unusually high). MFC uses the transformed parameter Gamma = 1/(gamma-1)",
            )
            self.warn(
                gamma > 1000,
                f"fluid_pp({i})%gamma = {gamma} implies physical gamma = {physical_gamma:.6f} (very close to 1). Did you enter the physical gamma instead of 1/(gamma-1)?",
            )
    ...
    def check_geometry_precision_simulation(self):
        ...
        self.prohibit(
            ...
            "Fully 3D cylindrical geometry (cyl_coord = T, p > 0) is not supported in single precision (--single)",
        )
    ...
    def _format_errors(self) -> str:
        ...
        for i, err in enumerate(self.errors, 1):
            ...
            if "must be positive" in err_lower or "must be set" in err_lower:
                lines.append(
                    "     [dim]Check that this required parameter is defined in your case file[/dim]"
                )
                continue
            if "boundary" in err_lower or "bc_" in err_lower:
                lines.append(
                    "     [dim]Common BC values: -1 (periodic), -2 (reflective), -3 (extrapolation)[/dim]"
                )
                continue
            ...
            for param_name, constraint in CONSTRAINTS.items():
                if not re.search(
                    r"\b" + re.escape(param_name.lower()) + r"\b", err_lower
                ):
                    continue
                ...
                if labels:
                    items = [
                        f"{v} ({labels[v]})" if v in labels else str(v) for v in choices
                    ]
        ...
        lines.append("")
        lines.append(
            "[dim]Tip: Run './mfc.sh validate case.py' for detailed validation[/dim]"
        )
...
def validate_case_constraints(
    params: Dict[str, Any], stage: str = "simulation"
) -> List[str]:
