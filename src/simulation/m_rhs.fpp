...
contains
    ...
    impure subroutine s_compute_rhs(q_cons_vf, q_T_sf, q_prim_vf, bc_type, rhs_vf, pb_in, rhs_pb, mv_in, rhs_mv, t_step, &
        ...
        ! Additional Physics and Source Temrs Additions for acoustic_source
        if (acoustic_source) then
            call nvtxStartRange("RHS-ACOUSTIC-SRC")
            call s_acoustic_src_calculations(q_cons_qp%vf(1:sys_size), q_prim_qp%vf(1:sys_size), t_step, rhs_vf)
            call nvtxEndRange
        end if
            ...
            if (cyl_coord .and. ((bc_y%beg == -2) .or. (bc_y%beg == -14))) then
                if (viscous .or. dummy) then
                    if (p > 0) then
                        ...
                        call s_compute_viscous_stress_cylindrical_boundary(q_prim_vf, dq_prim_dx_vf(mom_idx%beg:mom_idx%end), &
                            & dq_prim_dy_vf(mom_idx%beg:mom_idx%end), dq_prim_dy_vf(mom_idx%beg:mom_idx%end), tau_Re_vf, &
                            & idwbuff(1), idwbuff(2), idwbuff(3))
                    end if
