...
contains
    ...
    subroutine s_gaussian_atomic(rad, vel, pos, force_p, gauSum, gauSum_sources, cell, updatedvar, kcomp, ind_start, ind_end)
        ...
        ! real(wp) :: addFun_alphap, addFun_alphap_vp_x, addFun_alphap_vp_y, addFun_alphap_vp_z real(wp) :: addFun2_x, addFun2_y,
        ! addFun2_z, addFun_E
        real(wp)               :: addFun
        real(wp), dimension(3) :: nodecoord, center
        ...
        do dk = dk_beg, dk_end
            do dj = dj_beg, dj_end
                do di = di_beg, di_end
                    ...
                    ! addFun_alphap = weight*volpart $:GPU_ATOMIC(atomic='update') updatedvar(1)%sf(cellijk(1), cellijk(2),
                    ! cellijk(3)) = & updatedvar(1)%sf(cellijk(1), cellijk(2), cellijk(3)) & + real(addFun_alphap, kind=stp)

                    ! !Update particle momentum field(x) addFun_alphap_vp_x = weight*volpart*vp_x $:GPU_ATOMIC(atomic='update')
                    ! updatedvar(2)%sf(cellijk(1), cellijk(2), cellijk(3)) = & updatedvar(2)%sf(cellijk(1), cellijk(2), cellijk(3))
                    ! & + real(addFun_alphap_vp_x, kind=stp)

                    ! !Update particle momentum field(y) addFun_alphap_vp_y = weight*volpart*vp_y $:GPU_ATOMIC(atomic='update')
                    ! updatedvar(3)%sf(cellijk(1), cellijk(2), cellijk(3)) = & updatedvar(3)%sf(cellijk(1), cellijk(2), cellijk(3))
                    ! & + real(addFun_alphap_vp_y, kind=stp)

                    ! if (num_dims == 3) then !Update particle momentum field(z) addFun_alphap_vp_z = weight*volpart*vp_z
                    ! $:GPU_ATOMIC(atomic='update') updatedvar(4)%sf(cellijk(1), cellijk(2), cellijk(3)) = &
                    ! updatedvar(4)%sf(cellijk(1), cellijk(2), cellijk(3)) & + real(addFun_alphap_vp_z, kind=stp) end if

                    ! !Update upx^2 addFun_alphap_vp_x = weight*volpart*vp_x**2 $:GPU_ATOMIC(atomic='update')
                    ! updatedvar(5)%sf(cellijk(1), cellijk(2), cellijk(3)) = & updatedvar(5)%sf(cellijk(1), cellijk(2), cellijk(3))
                    ! & + real(addFun_alphap_vp_x, kind=stp)

                    ! !Update upy^2 addFun_alphap_vp_y = weight*volpart*vp_y**2 $:GPU_ATOMIC(atomic='update')
                    ! updatedvar(6)%sf(cellijk(1), cellijk(2), cellijk(3)) = & updatedvar(6)%sf(cellijk(1), cellijk(2), cellijk(3))
                    ! & + real(addFun_alphap_vp_y, kind=stp)

                    ! if (num_dims == 3) then !Update upz^2 addFun_alphap_vp_z = weight*volpart*vp_z**2
                    ! $:GPU_ATOMIC(atomic='update') updatedvar(7)%sf(cellijk(1), cellijk(2), cellijk(3)) = &
                    ! updatedvar(7)%sf(cellijk(1), cellijk(2), cellijk(3)) & + real(addFun_alphap_vp_z, kind=stp) end if

                    ! if (lag_params%solver_approach == 2) then weight = func/gauSum_sources

                    ! !Update x-momentum source term addFun2_x = weight*fp_x $:GPU_ATOMIC(atomic='update')
                    ! updatedvar(8)%sf(cellijk(1), cellijk(2), cellijk(3)) = & updatedvar(8)%sf(cellijk(1), cellijk(2), cellijk(3))
                    ! & + real(addFun2_x, kind=stp)

                    ! !Update y-momentum source term addFun2_y = weight*fp_y $:GPU_ATOMIC(atomic='update')
                    ! updatedvar(9)%sf(cellijk(1), cellijk(2), cellijk(3)) = & updatedvar(9)%sf(cellijk(1), cellijk(2), cellijk(3))
                    ! & + real(addFun2_y, kind=stp)

                    ! if (num_dims == 3) then !Update z-momentum source term addFun2_z = weight*fp_z $:GPU_ATOMIC(atomic='update')
                    ! updatedvar(10)%sf(cellijk(1), cellijk(2), cellijk(3)) = & updatedvar(10)%sf(cellijk(1), cellijk(2),
                    ! cellijk(3)) & + real(addFun2_z, kind=stp) end if

                    ! !Update energy source term addFun_E = 0._wp $:GPU_ATOMIC(atomic='update') updatedvar(11)%sf(cellijk(1),
                    ! cellijk(2), cellijk(3)) = & updatedvar(11)%sf(cellijk(1), cellijk(2), cellijk(3)) & + real(addFun_E, kind=stp)
    ...
    ! Created Feb. 1, 2024 - T.L. Jackson Modified 3/6/24 - Balachandar
    !
    ...
    !
    ! Sam - TODO: either couple the projection filter to the fluctuation filter or completely decouple them. Right now they use the
    ! same filter width and are assumed to be the same.
    ...
    function QS_ModifiedParmar(rho, cson, mu_fluid, gamma, vmag, dp, volume_fraction) result(beta)
        ...
        b1 = 5.81*phi/((1.0 - phi)**2) + 0.48*(phi**(1._wp/3._wp))/((1.0 - phi)**3)

        b2 = ((1.0 - phi)**2)*(phi**3)*re*(0.95 + 0.61*(phi**3)/((1.0 - phi)*2))

        b3 = min(sqrt(20.0_wp*mp), 1.0_wp)*(5.65*phi - 22.0*(phi**2) + 23.4*(phi**3))*(1 + tanh((mp - (0.65 - 0.24*phi))/0.35))
    ...
    !
    ! NOTE: Re<45 Rarefied fomu_fluidla of Loth et al has been redefined by Balachandar to avoid singularity as Ma -> 0.
    ...
    function QS_Gidaspow(rho, cson, mu_fluid, gamma, vmag, dp, volume_fraction) result(beta)
        ...
        if (phif < 0.8) then
            beta = 150.0*((phi**2)*mu_fluid)/(phif*dp**2) + 1.75*(rho*phi*vmag/dp)
        else
