...
contains
    ...
    subroutine s_ibm_correct_state(q_cons_vf, q_prim_vf, pb_in, mv_in)
        ...
        if (num_inner_gps > 0) then
            $:GPU_PARALLEL_LOOP(private='[i, physical_loc, dyn_pres, alpha_rho_IP, alpha_IP, vel_g, rho, gamma, pi_inf, Re_K, &
                                & innerp, j, k, l, q]')
    ...
    subroutine s_find_num_ghost_points(num_gps_out, num_inner_gps_out)
        ...
        $:GPU_PARALLEL_LOOP(private='[i, j, k, ii, jj, kk, is_gp]', copy='[num_gps_local, num_inner_gps_local]', &
    ...
    subroutine s_compute_ib_forces(q_prim_vf, fluid_pp)
        ...
        $:GPU_PARALLEL_LOOP(private='[ib_idx, fluid_idx, radial_vector, local_force_contribution, cell_volume, &
                            ...
                            & viscous_stress_div_2, viscous_cross_1, viscous_cross_2, dx, dy, dz]', copy='[forces, torques]', &
                            & copyin='[ib_markers, patch_ib, dynamic_viscosities]', collapse=3)
        do i = 0, m
            do j = 0, n
                do k = 0, p
                    ...
                    if (ib_idx /= 0) then
                        ...
                        ! get the viscous stress and add its contribution if that is considered TODO :: This is really bad code
                        if (viscous) then
                            ...
                            viscous_stress_div = (viscous_cross_2 - viscous_cross_1)/(2._wp*dx) &
                                                  &  ! get the x derivative of the cross product
                            local_torque_contribution(1:3) = local_torque_contribution(1:3) + viscous_stress_div(1, &
                                                      & 1:3)  ! apply the cross product derivative to the torque
