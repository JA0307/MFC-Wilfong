...
contains
    ...
    impure subroutine s_initialize_particles_EL_module(q_cons_vf, bc_type)
        ...
        do i = 1, 1
            @:ALLOCATE(q_particles_old(i)%sf(idwbuff(1)%beg:idwbuff(1)%end, idwbuff(2)%beg:idwbuff(2)%end, &
                       & idwbuff(3)%beg:idwbuff(3)%end))
    ...
    impure subroutine s_restart_bubbles(part_id, save_count)
        ...
        if (part_id > 0) then
            ...
            do i = 1, part_id
                ...
                ! bub_dphidt(i) = MPI_IO_DATA_lag_bubbles(i, 16) gas_p(i, 1) = MPI_IO_DATA_lag_bubbles(i, 17) gas_mv(i, 1) =
                ! MPI_IO_DATA_lag_bubbles(i, 18)
                particle_mass(i) = MPI_IO_DATA_lag_bubbles(i, 19)
                ! gas_betaT(i) = MPI_IO_DATA_lag_bubbles(i, 20) gas_betaC(i) = MPI_IO_DATA_lag_bubbles(i, 21)
                cell = -buff_size
        ...
        if (part_id > 0) then
            ...
            do k = 1, n_el_particles_loc
                if (particle_in_domain_physical(particle_pos(k,1:3,1))) then
                    ...
                    ! MPI_IO_DATA_lag_bubbles(i, 16) = bub_dphidt(k) MPI_IO_DATA_lag_bubbles(i, 17) = gas_p(k, 1)
                    ! MPI_IO_DATA_lag_bubbles(i, 18) = gas_mv(k, 1)
                    MPI_IO_DATA_lag_bubbles(i, 19) = particle_mass(k)
                    ! MPI_IO_DATA_lag_bubbles(i, 20) = gas_betaT(k) MPI_IO_DATA_lag_bubbles(i, 21) = gas_betaC(k)
                    i = i + 1
                end if
            end do
