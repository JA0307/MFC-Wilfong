...
contains
    ...
    !> This subroutine performs the MPI communication for lagrangian particles/ particles.
            ...
            if (p_send_counts(i, j, k) > 0 .and. abs(i) + abs(j) + abs(k) /= 0 .and. abs(i) + abs(j) + abs(k) /= 0) then
                ...
                do q = 0, p_send_counts(i, j, k) - 1
                    ...
                    call MPI_Pack(fqs_fluct(particle_id,:), 3, mpi_p, p_send_buff(send_offset), p_buff_size, position, &
                                  & MPI_COMM_WORLD, ierr)
                    ! call MPI_Pack(gas_betaT(particle_id), 1, mpi_p, p_send_buff(send_offset), p_buff_size, position,
                    ! MPI_COMM_WORLD, ierr) call MPI_Pack(gas_betaC(particle_id), 1, mpi_p, p_send_buff(send_offset), p_buff_size,
                    ! position, MPI_COMM_WORLD, ierr) call MPI_Pack(bub_dphidt(particle_id), 1, mpi_p, p_send_buff(send_offset),
                    ! p_buff_size, position, MPI_COMM_WORLD, ierr)
                    do r = 1, 2
                        ! call MPI_Pack(gas_p(particle_id, r), 1, mpi_p, p_send_buff(send_offset), p_buff_size, position,
                        ! MPI_COMM_WORLD, ierr) call MPI_Pack(gas_mv(particle_id, r), 1, mpi_p, p_send_buff(send_offset),
                        ! p_buff_size, position, MPI_COMM_WORLD, ierr)
                        call MPI_Pack(rad(particle_id, r), 1, mpi_p, p_send_buff(send_offset), p_buff_size, position, &
                                      & MPI_COMM_WORLD, ierr)
                        ! call MPI_Pack(rvel(particle_id, r), 1, mpi_p, p_send_buff(send_offset), p_buff_size, position,
                        ! MPI_COMM_WORLD, ierr)
                    ...
                    do r = 1, lag_num_ts
                        call MPI_Pack(drad(particle_id, r), 1, mpi_p, p_send_buff(send_offset), p_buff_size, position, &
                                      & MPI_COMM_WORLD, ierr)
                        ! call MPI_Pack(drvel(particle_id, r), 1, mpi_p, p_send_buff(send_offset), p_buff_size, position,
                        ! MPI_COMM_WORLD, ierr) call MPI_Pack(dgasp(particle_id, r), 1, mpi_p, p_send_buff(send_offset),
                        ! p_buff_size, position, MPI_COMM_WORLD, ierr) call MPI_Pack(dgasmv(particle_id, r), 1, mpi_p,
                        ! p_send_buff(send_offset), p_buff_size, position, MPI_COMM_WORLD, ierr)
            ...
            if (p_recv_counts(i, j, k) > 0 .and. abs(i) + abs(j) + abs(k) /= 0) then
                ...
                do q = 0, p_recv_counts(i, j, k) - 1
                    ...
                    call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, fqs_fluct(particle_id,:), 3, mpi_p, &
                                    & MPI_COMM_WORLD, ierr)
                    ! call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, gas_betaT(particle_id), 1, mpi_p,
                    ! MPI_COMM_WORLD, ierr) call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, gas_betaC(particle_id),
                    ! 1, mpi_p, MPI_COMM_WORLD, ierr) call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position,
                    ! bub_dphidt(particle_id), 1, mpi_p, MPI_COMM_WORLD, ierr)
                    do r = 1, 2
                        ! call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, gas_p(particle_id, r), 1, mpi_p,
                        ! MPI_COMM_WORLD, ierr) call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, gas_mv(particle_id,
                        ! r), 1, mpi_p, MPI_COMM_WORLD, ierr)
                        call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, rad(particle_id, r), 1, mpi_p, &
                                        & MPI_COMM_WORLD, ierr)
                        ! call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, rvel(particle_id, r), 1, mpi_p,
                        ! MPI_COMM_WORLD, ierr)
                    ...
                    do r = 1, lag_num_ts
                        call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, drad(particle_id, r), 1, mpi_p, &
                                        & MPI_COMM_WORLD, ierr)
                        ! call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, drvel(particle_id, r), 1, mpi_p,
                        ! MPI_COMM_WORLD, ierr) call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, dgasp(particle_id,
                        ! r), 1, mpi_p, MPI_COMM_WORLD, ierr) call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position,
                        ! dgasmv(particle_id, r), 1, mpi_p, MPI_COMM_WORLD, ierr)
